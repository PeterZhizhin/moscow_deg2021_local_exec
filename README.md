...
1. Устанавливаем Docker и поднимаем все сервисы через `docker compose up --detach`.
1. Переходим по адресу [http://localhost:8004/election](http://localhost:8004/election) и пробуем голосовать.
Если всё будет правильно, то у вас получится получить код по SMS. **Сами SMS не отправляются**, надо посмотреть в stderr сервиса `form`: `docker logs "название контейнера с form"`,
